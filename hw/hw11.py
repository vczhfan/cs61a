...
def tokenize(line):
    ...
    for x in [x for x in line.split(' ') if x != '']:
        if x in ALL_BRACKETS:
            ...
            if not coerce_to_number(x):
                raise ValueError("invalid token {0}".format(x))
            tokens_list.append(coerce_to_number(x))
    return tokens_list
...
def isvalid(tokens):
    ...
    if len(tokens) == 0:
        return False
    elif len(tokens) == 1 and type(tokens[0]) in (int, float):
        return True
    else:
        for x in tokens:
            if x in list(LEFT_RIGHT.keys()):
                stack.append(x)
            elif x in list(LEFT_RIGHT.values()):
                if len(stack) > 0 and LEFT_RIGHT[stack[-1]] == x:
                    stack.pop()
                    a_valid_expr = True
                elif len(stack) == 0 and a_valid_expr:
                    return True
                else:
                    return False
            else:
...
def brack_read(tokens):
    ...
    x = tokens.pop(0)
    i = coerce_to_number(x)
    if i != None:
        return i
    if x in ALL_BRACKETS:
        rest = read_tail(tokens)
        right = tokens.pop(0)
        if right != LEFT_RIGHT[x]:
            raise SyntaxError('unexpected ' + right)
        return Pair(BRACKETS[(x, right)], rest)
    else:
        raise SyntaxError('unexpected ' + x)
